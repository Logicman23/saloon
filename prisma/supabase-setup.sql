-- ============================================================================
--  Sana's Beauty Saloon Management System — full Supabase setup
--  Paste whole into the Supabase SQL Editor and Run. Idempotent: safe to run
--  on an empty database, and safe to re-run.
--
--  TABLE NAME MAP — the names below are what Prisma generates from
--  prisma/schema.prisma. They are not negotiable: the deployed app queries
--  these exact names, so renaming them to `User`/`Invoice`/etc. would make
--  every page 500.
--
--      you asked for        actual table        note
--      -----------------    ----------------    ------------------------------
--   1. User            ->   users               + user_roles (the `role` FK)
--   2. Client          ->   clients
--   3. Service         ->   services            `name` is the column `title`
--   4. Appointment     ->   appointments        services attach via join table
--   5. Inventory       ->   inventory
--   6. Invoice         ->   sales_invoices      payments are their own table
--   7. Expense         ->   expenses
--
--  Passwords are PBKDF2-HMAC-SHA512 / 210,000 iterations / 64-byte key, stored
--  as hex across TWO columns (password_hash + password_salt) — see
--  src/lib/auth/users.server.ts. Not bcrypt, and not computable in Postgres,
--  so the digests below were generated with node:crypto and verified against
--  the app's own verifyPassword().
-- ============================================================================

BEGIN;

-- ############################################################################
-- # 0. PREREQUISITES — enum types, roles, permissions, staff chairs
-- ############################################################################
...
-- Access levels. `key` MUST be ADMIN | CASHIER | STAFF — that is the `Role`
-- union in src/lib/auth/permissions.ts, and middleware compares against it.
...
CREATE TABLE IF NOT EXISTS "role_permissions" (
    ...
    CONSTRAINT "role_permissions_role_id_fkey"        FOREIGN KEY ("role_id")        REFERENCES "user_roles"("id")  ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT "role_permissions_permission_key_fkey" FOREIGN KEY ("permission_key") REFERENCES "permissions"("key") ON DELETE CASCADE ON UPDATE CASCADE
);

-- Staff = a chair in the salon (job title, commission, salary). Separate from
-- `users`, which is a login. A person can have both, linked by users.staff_id.
...
CREATE TABLE IF NOT EXISTS "users" (
    "id"                  TEXT         NOT NULL,
    "email"               TEXT         NOT NULL,          -- store lowercase only
    "name"                TEXT         NOT NULL,
    "password_hash"       TEXT         NOT NULL,          -- PBKDF2-SHA512 hex, 128 chars
    "password_salt"       TEXT         NOT NULL,          -- hex, used as a UTF-8 string
    "override_pin_hash"   TEXT,                           -- SHA-256 of manager PIN
    "active"              BOOLEAN      NOT NULL DEFAULT true,
    "last_login_at"       TIMESTAMP(3),
    "sessions_valid_from" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "failed_login_count"  INTEGER      NOT NULL DEFAULT 0,
    "locked_until"        TIMESTAMP(3),                   -- set after 10 failures
    "created_at"          TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updated_at"          TIMESTAMP(3) NOT NULL,
    "role_id"             TEXT         NOT NULL,
    "staff_id"            TEXT,
    CONSTRAINT "users_pkey" PRIMARY KEY ("id"),
    CONSTRAINT "users_role_id_fkey"  FOREIGN KEY ("role_id")  REFERENCES "user_roles"("id") ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT "users_staff_id_fkey" FOREIGN KEY ("staff_id") REFERENCES "staff"("id")      ON DELETE SET NULL  ON UPDATE CASCADE
);
CREATE UNIQUE INDEX IF NOT EXISTS "users_email_key"    ON "users"("email");
CREATE UNIQUE INDEX IF NOT EXISTS "users_staff_id_key" ON "users"("staff_id");  -- one login per chair
...
-- Written by every guarded server action (src/lib/actions/guard.ts) — required.
...
CREATE TABLE IF NOT EXISTS "clients" (
    "id"         TEXT         NOT NULL,
    "name"       TEXT         NOT NULL,
    "phone"      TEXT         NOT NULL,   -- the natural key; unique
    "email"      TEXT,
    "gender"     TEXT,
    "notes"      TEXT,
    "tags"       TEXT[],
...
-- ############################################################################
-- # 3. SERVICE  ->  "services"   (service name lives in the `title` column)
-- ############################################################################
...
CREATE TABLE IF NOT EXISTS "package_services" (
    ...
    CONSTRAINT "package_services_package_id_fkey" FOREIGN KEY ("package_id") REFERENCES "service_packages"("id") ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT "package_services_service_id_fkey" FOREIGN KEY ("service_id") REFERENCES "services"("id")         ON DELETE CASCADE ON UPDATE CASCADE
);

-- ############################################################################
-- # 4. APPOINTMENT  ->  "appointments"
-- #    One booking can cover several services, so there is no single
-- #    service_id column — services attach through appointment_services.
-- ############################################################################
...
CREATE TABLE IF NOT EXISTS "inventory" (
    ...
    "min_stock_alert" INTEGER       NOT NULL DEFAULT 5,   -- low-stock threshold
    "supplier"        TEXT,
    "created_at"      TIMESTAMP(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- Signed ledger: positive adds stock, negative consumes it.
CREATE TABLE IF NOT EXISTS "stock_movements" (
    ...
    CONSTRAINT "stock_movements_product_id_fkey" FOREIGN KEY ("product_id") REFERENCES "inventory"("id") ON DELETE CASCADE  ON UPDATE CASCADE,
    CONSTRAINT "stock_movements_staff_id_fkey"   FOREIGN KEY ("staff_id")   REFERENCES "staff"("id")     ON DELETE SET NULL ON UPDATE CASCADE
);
CREATE INDEX IF NOT EXISTS "stock_movements_product_id_at_idx" ON "stock_movements"("product_id", "at");

-- ############################################################################
-- # 6. INVOICE  ->  "sales_invoices"
-- #    Totals are snapshots written at checkout, never recomputed, so a later
-- #    price change cannot rewrite last month's books. Payment method lives on
-- #    `payments` because one bill can be settled by several methods.
-- ############################################################################
...
CREATE TABLE IF NOT EXISTS "sales_invoices" (
    ...
    CONSTRAINT "sales_invoices_client_id_fkey"           FOREIGN KEY ("client_id")           REFERENCES "clients"("id")      ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT "sales_invoices_appointment_id_fkey"      FOREIGN KEY ("appointment_id")      REFERENCES "appointments"("id") ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT "sales_invoices_created_by_staff_id_fkey" FOREIGN KEY ("created_by_staff_id") REFERENCES "staff"("id")        ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "invoice_lines" (
    ...
    "ref_id"          TEXT          NOT NULL,   -- source service / product / package id
    "name"            TEXT          NOT NULL,   -- denormalised: survives renames
    "unit_price"      DECIMAL(12,2) NOT NULL,
    "qty"             INTEGER       NOT NULL DEFAULT 1,
    "staff_id"        TEXT,                     -- per-line attribution for commission
    "commission_rate" DECIMAL(4,3)  NOT NULL DEFAULT 0,
    "line_discount"   DECIMAL(12,2) NOT NULL DEFAULT 0,
    CONSTRAINT "invoice_lines_pkey" PRIMARY KEY ("id"),
    CONSTRAINT "invoice_lines_invoice_id_fkey" FOREIGN KEY ("invoice_id") REFERENCES "sales_invoices"("id") ON DELETE CASCADE  ON UPDATE CASCADE,
    CONSTRAINT "invoice_lines_staff_id_fkey"   FOREIGN KEY ("staff_id")   REFERENCES "staff"("id")          ON DELETE SET NULL ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "payments" (
    ...
    CONSTRAINT "payments_invoice_id_fkey" FOREIGN KEY ("invoice_id") REFERENCES "sales_invoices"("id") ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "expenses" (
    ...
    CONSTRAINT "expenses_recorded_by_staff_id_fkey" FOREIGN KEY ("recorded_by_staff_id") REFERENCES "staff"("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
-- ############################################################################
-- # 10. SEED — login accounts
-- #
-- #     admin@sana.com    /  SanaAdmin#2026    ADMIN
-- #     ayesha@sana.com   /  Ayesha#2026       STAFF
-- #     cashier@sana.com  /  Cashier#2026      CASHIER
-- #
-- #     Emails must be lowercase — findUserByEmail() lowercases its input
-- #     before the lookup, so a capitalised row would never match.
-- ############################################################################
...
INSERT INTO "users" (
  ...
  (
    ...
    '0a4e3e70597a358b9447fa8a647aadf5b76dde95c8e4ab02e5f8cee6caa1cd28',  -- SHA-256 of override PIN 4826
    true, 0, now(), now(), now(),
    'role_admin', 'stf_sana'
  ),
  (
    'usr_ayesha_login',   -- not 'usr_ayesha': prisma/seed.ts reserves that id
    'ayesha@sana.com',
    'Ayesha Khan',
...
-- ############################################################################
-- # 11. SEED — services
-- #      `category` is an enum: Hair Care -> HAIR, Skin & Facial -> SKIN,
-- #      Bridal Makeup -> MAKEUP, Nail Bar -> NAILS, plus SPA.
-- ############################################################################
...
-- ############################################################################
-- # 13. SEED — clients, appointments, invoices, expenses
-- #      Enough live data that the dashboard and reports render on first load.
-- ############################################################################
...
-- ############################################################################
-- # 14. PRISMA BOOKKEEPING
-- #     Marks migration 0001_init as applied so a later `prisma migrate deploy`
-- #     does not try to re-create these tables. The checksum is the real
-- #     SHA-256 of prisma/migrations/0001_init/migration.sql.
-- ############################################################################
...
SELECT 'a1b2c3d4-0001-4000-8000-000000000001',
...
-- ############################################################################
-- # 15. HARDENING
-- #     Supabase exposes everything in `public` through PostgREST to the anon
-- #     key. These tables hold password hashes and financials and are only ever
-- #     read by Prisma over the direct Postgres connection, so switch RLS on
-- #     with no policies and revoke the API roles. The table owner (the role in
-- #     your DATABASE_URL) bypasses RLS, so the app is unaffected.
-- ############################################################################
...
-- ############################################################################
-- # 16. VERIFY — should return exactly three rows, hash_len 128, active true
-- ############################################################################

SELECT u.email,
       u.name,
       r.key            AS role,
       r.landing_path   AS lands_on,
       s.name           AS staff_chair,
